...
mod tests {
    ...
    */
    ...
    fn test_14_wasi_fs_read_violation() {
        ...
        let client = Client::new();
        let response = client.post(format!("{}/execute/fs-reader", BASE_URL)).send().unwrap();
    ...
    */
    ...
    fn test_15_wasi_env_var_isolation() {
        ...
        let client = Client::new();
        let response = client.post(format!("{}/execute/env-reader", BASE_URL)).send().unwrap();
        assert_eq!(response.status(), StatusCode::OK);
    ...
    */
    ...
    fn test_16_wasi_network_socket_violation() {
        ...
        let client = Client::new();
        let response = client.post(format!("{}/execute/net-guest", BASE_URL)).send().unwrap();
        let text = response.text().unwrap();
    ...
    */
    ...
    fn test_17_memory_maximum_size_trap() {
        ...
        let client = Client::new();
        let response = client.post(format!("{}/execute/memory-hog", BASE_URL)).send().unwrap();
    ...
    */
    ...
    fn test_18_stdout_pipe_limit_truncation() {
        ...
        let client = Client::new();
        let response = client.post(format!("{}/execute/stdout-spammer", BASE_URL)).send().unwrap();
    ...
    */
    ...
    async fn test_20_db_lifecycle_success_record() {
        let client = Client::new();
        client.post(format!("{}/execute/hello-world", BASE_URL)).body("DB_Check").send().unwrap();

        let pool = sqlx::PgPool::connect("postgres://postgres:postgres@localhost:5432/serverless").await.unwrap();
        let record = sqlx::query!("SELECT status_code, stdout_snippet FROM executions ORDER BY created_at DESC LIMIT 1").fetch_one(&pool).await.unwrap();
    ...
    */
    ...
    async fn test_21_db_lifecycle_duration_ms() {
        let client = Client::new();
        client.post(format!("{}/execute/hello-world", BASE_URL)).send().unwrap();

        let pool = sqlx::PgPool::connect("postgres://postgres:postgres@localhost:5432/serverless").await.unwrap();
        let record = sqlx::query!("SELECT duration_ms FROM executions ORDER BY created_at DESC LIMIT 1").fetch_one(&pool).await.unwrap();
    ...
    */
    ...
    async fn test_22_db_lifecycle_error_logging() {
        ...
        let _ = client.post(format!("{}/execute/panic-guest", BASE_URL)).send().unwrap();

        let pool = sqlx::PgPool::connect("postgres://postgres:postgres@localhost:5432/serverless").await.unwrap();
        let record = sqlx::query!("SELECT status_code, error_message FROM executions ORDER BY created_at DESC LIMIT 1").fetch_one(&pool).await.unwrap();

        assert_eq!(record.status_code, Some(101));
        assert!(record.error_message.unwrap().contains("Wasm exited with code"));
    }
    */
    ...
    async fn test_23_db_stdout_snippet_truncation() {
        ...
        let client = Client::new();
        client.post(format!("{}/execute/long-output-guest", BASE_URL)).send().unwrap();

        let pool = sqlx::PgPool::connect("postgres://postgres:postgres@localhost:5432/serverless").await.unwrap();
        let record = sqlx::query!("SELECT stdout_snippet FROM executions ORDER BY created_at DESC LIMIT 1").fetch_one(&pool).await.unwrap();
    ...
    */
    ...
    async fn test_24_db_pre_check_failure_no_log() {
        ...
        let pool = sqlx::PgPool::connect("postgres://postgres:postgres@localhost:5432/serverless").await.unwrap();
        let count_before: i64 = sqlx::query_scalar!("SELECT count(*) FROM executions").fetch_one(&pool).await.unwrap();

        let client = Client::new();
        client.post(format!("{}/execute/does_not_exist", BASE_URL)).send().unwrap();

        let count_after: i64 = sqlx::query_scalar!("SELECT count(*) FROM executions").fetch_one(&pool).await.unwrap();
